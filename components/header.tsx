...
import SearchNavigation from "@/components/searchnav"
...
const Header = () => {
    ...
    const getPageTitle = (pathname: string) => {
      ...
      if (pathname.startsWith('/admin/users/edit')) return 'Edit User';
      if (pathname.startsWith('/admin/users')) return 'Manage Users';
      if (pathname.startsWith('/admin/groups/add')) return 'Add Group';
      if (pathname.startsWith('/admin/groups/edit')) return 'Edit Group';
      if (pathname.startsWith('/admin/groups')) return 'Manage Groups';
      if (pathname.startsWith('/admin/rules')) return 'Rules';
      if (pathname.startsWith('/admin')) return 'Admin Page';
      if (pathname.startsWith('/alert')) return 'Alert Page';
      if (pathname.startsWith('/lists')) return 'Lists to filter';
      if (pathname.startsWith('/rules')) return 'Correlation Rules';
    ...
    return (
        <header className="header relative top-0 z-99999 flex w-full border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 lg:border-b">
            <div className="flex grow flex-col items-center justify-between lg:flex-row m-4 lg:px-6">
                <div className="flex w-full items-center justify-between gap-2">
                    <h1 className="text-3xl font-semibold text-gray-800 dark:text-white/90">{pageTitle ? pageTitle: ""}</h1>
                ...
                <div className="flex items-center space-x-6 pr-20">
                    ...
                    <div className="flex items-center space-x-2 cursor-pointer">{username? username : ""}</div>
                </div>
            </div>
        </header>
    );
